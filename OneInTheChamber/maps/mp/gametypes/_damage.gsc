...
#include maps\mp\gametypes\berettadamage;
#include common_scripts\utility;


getHitLocDamage( sHitLoc, sWeapon, iDamage )
{
	if(isSubStr(sWeapon, getDvar("oneshot_weapon"))) {
		return getWepDamageberetta(sHitLoc, iDamage);
	}
...
realismModHitLegs(player, sHitLoc, eInflictor, attacker, iDamage, sWeapon, vDir, sMeansOfDeath)
{
	switch( sHitLoc )
	{
		...
		case "left_foot":
			...
			player.lastStandParams = lastStandParams;
	
			player.inLastStand = true;
			player.inFinalStand = true;
			player _disableUsability();
			player thread enableLastStandWeapons();

			player thread lastStandTimer( 900, true );
		default:
			return;
	}
}
...
{	
	...
	iDamage = getHitLocDamage(sHitLoc, sWeapon, iDamage);
	...
	//realismModHitLegs(victim, sHitLoc, eInflictor, eAttacker, iDamage, sWeapon, vDir, sMeansOfDeath);
...
{	
	...
	*/
	...
	else // normal last stand
	{
		...
		{
			pistolWeapon = getDvar("oneshot_weapon");
